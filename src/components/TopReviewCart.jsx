...
const TopReviewCart = ({ review }) => {
 ...
 const [relativeTime, setRelativeTime] = useState("");
  ...
  return (
    <div className="border border-base-300 rounded-2xl flex flex-col justify-between overflow-hidden">
      <div className="flex items-center gap-3 px-3 pt-3">
        ...
        <h2 className="text-xs flex-1 text-right ">{relativeTime}</h2>
      </div>
      ...
      <Link to={`/service-details/${serviceId}`} className="flex items-center gap-3 cursor-pointer border-base-300  p-3 border-t hover:bg-secondaryTextColor/5">
        <img
          className="w-9 h-9 rounded-full object-cover"
          onError={handleImage}
          src={serviceLogo}
